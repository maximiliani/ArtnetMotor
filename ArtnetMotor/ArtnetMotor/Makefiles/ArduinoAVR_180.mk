...
ifneq ($(findstring LITTLEROBOTFRIENDS,$(GCC_PREPROCESSOR_DEFINITIONS)),)
    ...
    PLATFORM         := Arduino
    PLATFORM_TAG      = ARDUINO=10812 ARDUINO_ARCH_AVR ARDUINO_$(BUILD_BOARD) EMBEDXCODE=$(RELEASE_NOW) ARDUINO_$(ARDUINO_TAG)
    APPLICATION_PATH := $(ARDUINO_PATH)
...
# Option 1 - Tools from /Applications/Arduino.app
APP_TOOLS_PATH     := $(APPLICATION_PATH)/hardware/tools/avr/bin
# Option 2 - Tools from ~/Library/Arduino15/packages/arduino/tools
#APP_TOOLS_PATH      := $(ARDUINO_AVR_PATH)/tools/avr-gcc/$(ARDUINO_GCC_AVR_RELEASE)/bin
...
APP_LIB_H_SRC  += $(foreach dir,$(avr165_20),$(wildcard $(dir)/*.hpp)) # */
...
avr166_30              = $(filter-out %main.cpp, $(shell find $(CORE_LIB_PATH) -name \*.cpp))
